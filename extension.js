  ...
  class MprisLabel extends PanelMenu.Button {
    _init() {
      ...
      this.settings.connect(
        ...
        this._onPaddingChanged.bind(this)
      );
      this.settings.connect(
      ...
      this.settings.connect(
        ...
        this._updateTrayPosition.bind(this)
      );
      this.settings.connect(
      ...
      this._repositionTimeout = GLib.timeout_add_seconds(
    ...
    _onPaddingChanged() {
      ...
      if (SHOW_ICON) {
        ...
        else LEFT_PADDING = Math.max(0, LEFT_PADDING - 5);
      }
    ...
    _determineColors() {
      ...
      let mixedColor = Clutter.Color.new(
        new_channels[0],
        new_channels[1],
        new_channels[2],
        new_channels[3]
    ...
    _onClick(event) {
      const REPOSITION_ON_BUTTON_PRESS = this.settings.get_boolean(
        "reposition-on-button-press"
      );

      if (REPOSITION_ON_BUTTON_PRESS) this._updateTrayPosition(); //force tray position update on button press
    ...
    _activateButton(option) {
      ...
      switch (value) {
        case "play-pause":
          if (this.player) this.player.toggleStatus();
          break;
        case "next-track":
          if (this.player) this.player.goNext();
          break;
        case "prev-track":
          if (this.player) this.player.goPrevious();
          break;
        case "activate-player":
          if (this.player) this.player.activatePlayer();
          break;
        case "open-menu":
          this._buildMenu();
          this.menu.toggle();
    ...
    _getStream() {
      if (!this.player || !this.player.identity) return;
      ...
      streamList.forEach((stream) => {
        if (
          stream.get_name() &&
          stream.get_name().toLowerCase() == this.player.identity.toLowerCase()
        )
          this.stream.push(stream);
      });

      if (this.stream.length > 0) return this.stream;
    ...
    _buildMenu() {
      const AUTO_SWITCH_TO_MOST_RECENT = this.settings.get_boolean(
      ...
      this.players.list.forEach((player) => {
        ...
        if (AUTO_SWITCH_TO_MOST_RECENT) {
          if (!this.unfocusColor) this._determineColors();
      ...
      if (this.players.list.length > 0) {
        let settingsMenuItem = new PopupMenu.PopupMenuItem(
    ...
    _refresh() {
      ...
      if (this.player != prevPlayer) this._getStream();
    ...
    _setIcon() {
      ...
      if (
        ...
        !this.player ||
        this.label.get_text() == "" ||
        this.label.get_text() == PLACEHOLDER
      )
        return;
      ...
      if ((this.icon != null) | undefined) {
        if (ICON_PLACE == "right") this.box.add_child(this.icon);
    ...
    _setText() {
      try {
        if (this.player == null || undefined) this.label.set_text("");
        else this.label.set_text(buildLabel(this.players));
      } catch (err) {
        log("Mpris Label: " + err);
        this.label.set_text("");
      }
    ...
    _disable() {
      if (this.icon) this.box.remove_child(this.icon);
